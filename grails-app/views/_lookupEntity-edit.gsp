<html>
  ...
  <title><g:message code="${beanName}.edit.label" args="[entityName]" /></title>
  <g:render template="/iconAutocompleteScript" plugin="crm-base"/>
</head>
<body>
  <div class="dialog">
    <h1><g:message code="${beanName}.edit.title" args="[entityName, bean]" /></h1>
    ...
    <g:form>
      ...
      <div class="buttons">
        <crm:button class="positive" action="update" icon="disk" message="${beanName}.button.save.label"/>
        <crm:button class="negative" action="delete" icon="delete" message="${beanName}.button.delete.label" confirm="true"/>
        <crm:link action="show" id="${bean.id}" icon="cancel" message="${beanName}.button.cancel.label"/>
      </div>
